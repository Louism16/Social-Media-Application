...
import spark.Spark;
import edu.lehigh.cse216.lom225.backend.Database.DataRow;
import edu.lehigh.cse216.lom225.backend.DataRowLite;
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        Spark.put("/ideas/:id", (request, response) -> {
            ...
            response.status(200);
            response.type("application/json");
            int result = db.updateOne(req.mMessage, req.mLikes, idx);
            //int result = db.updateLikes(req.mLikes);
            if (result == -1) {
        ...
        // Like a post
        Spark.post("/ideas/:id", (request, response) -> {
            try {
                int postId = Integer.parseInt(request.params("id"));
                // Retrieve the post by its ID
                DataRow likedPost = db.selectOne(postId);
                if (likedPost != null) {
                    // Increment the like count for the post
                    likedPost.incrementLikes();
                    // Return a success response
                    return new StructuredResponse("ok", "Post liked successfully", likedPost);
                } else {
                    response.status(404); // Not Found
                    return new StructuredResponse("error", "Post not found", null);
                }
            } catch (NumberFormatException e) {
                response.status(400); // Bad Request
                return new StructuredResponse("error", "Invalid post ID", null);
            }
        });

    //    //Get what ideas are liked
    //    Spark.get("/ideas", (request, response) -> {
    //         response.status(200);
    //         response.type("application/json");
    //         ArrayList<DataRow> posts = db.selectAll(); // Retrieve all posts
    //         // Create an ArrayList to store posts with like counts
    //         ArrayList<StructuredResponse> postsWithLikes = new ArrayList<>();
    //         for (DataRow post : posts) {
    //             // Create a structured response for each post with Post_ID, Likes, and Message
    //             StructuredResponse postResponse = new StructuredResponse(
    //                 "ok",
    //                 "Post retrieved successfully",
    //                 new DataRowLite(post.Post_ID, post.mLikes, post.mMessage)
    //             );
    //             postsWithLikes.add(postResponse);
    //         }
    //         // Return the list of posts with like counts as JSON
    //         return new StructuredResponse("ok", "Posts retrieved successfully", postsWithLikes);
    //     });
    }
}